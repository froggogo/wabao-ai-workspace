    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-3xl px-6 py-8">
        ...
        <div className="mt-5 flex gap-1 border-b border-slate-200">
          ...
          <TabBtn active={tab === "audit"} onClick={() => setTab("audit")}>审核记录</TabBtn>
        </div>
    ...
    <div className="mt-6 space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="mt-6 space-y-4">
          ...
          <ReadonlyRow label="套餐" value={plan} />
        </div>
...
function AuditTab() {
  const [records, setRecords] = useState<ModerationRecord[] | null>(null);
  ...
  useEffect(() => {
    ...
    setError("");
    api.moderation
      .records(onlyFlagged ? { flagged: true } : undefined)
      .then(setRecords)
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {records === null && !error && (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-slate-400">加载中…</td>
              </tr>
            )}
            {error && (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-red-400">{error}</td>
              </tr>
            )}
            {records?.map((r) => (
            ...
            {records && records.length === 0 && (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-slate-400">暂无审核记录</td>
              </tr>
            )}
          </tbody>
        </table>
