...
export function Pricing() {
  const currentPlan = useApp((s) => s.userPlan);
  const upgradePlan = useApp((s) => s.upgradePlan);
            ...
            <tr className="border-b border-slate-100">
              ...
              {planIds.map((id) => (
                ...
                >
                  {PLAN_LABELS[id]}
                  {id === currentPlan && (
                    <div className="text-[10px] font-normal text-brand-500">当前</div>
                  )}
                </th>
              ))}
            </tr>
...
function UpgradeModal({
  ...
  const unitLabel = plan.unit === "seat" ? "/人·月" : "/月";
  const yearlyTotal =
    plan.priceYearlyPerMonth != null ? plan.priceYearlyPerMonth * 12 : null;
        ...
        {isEnterprise ? (
          ...
          <>
            <div className="mt-3 flex items-center justify-between rounded-xl bg-slate-50 p-4">
              <div>
                <div className="text-sm text-slate-500">{plan.name} · {cycle === "yearly" ? "年付" : "月付"}</div>
